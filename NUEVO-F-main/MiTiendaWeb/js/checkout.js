...
// Variable global para controlar si hay un envío en proceso
...
function manejarEnvioFormulario(evento) {
    ...
    // Deshabilitar completamente el evento beforeunload
    ...
    if (!metodoPago) {
        ...
        resetearFormulario(submitButton);
        return;
    }
    ...
    // Si es pickup, forzar el método de pago a efectivo
    const metodoPagoFinal = metodoEntrega === 'pickup' ? 'efectivo' : metodoPago;
    ...
    // Generar el número de pedido una sola vez
    ...
    fetch('http://127.0.0.1:3000/api/pedidos', {
        ...
        body: JSON.stringify({
            ...
            entrega: {
                ...
                direccion: pedido.metodoEntrega === 'pickup' ? 
                    document.querySelector('input[name="pickup-location"]:checked')?.value || 'san-patricio' :
                    {
                        calle: pedido.datosEnvio.direccion,
                        ciudad: pedido.datosEnvio.ciudad,
                        estado: pedido.datosEnvio.estado,
                        cp: pedido.datosEnvio.cp
                    }
            },
    ...
    .then(response => {
        if (response.status === 409) {
            throw new Error('Este número de pedido ya existe. Por favor, intenta nuevamente.');
        }
        if (!response.ok) {
    ...
    .then(data => {
        ...
        // Redirigir usando replace y forzando la navegación
        const confirmacionUrl = `confirmacion.html?pedido=${data.numero_pedido}`;
        window.location.replace(confirmacionUrl);
        
        // Forzar la navegación después de un breve retraso si replace no funciona
        setTimeout(() => {
            window.location.href = confirmacionUrl;
        }, 100);
    })
    .catch(error => {
        alert(error.message || 'Hubo un problema al guardar el pedido. Intenta de nuevo.');
        console.error(error);
        resetearFormulario(submitButton);
    });
}
...
function resetearFormulario(submitButton) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (metodoEntrega && pickupLocations && direccionInputs) {
    ...
    // Manejar el envío del formulario (solo un manejador)
    const checkoutForm = document.getElementById('shipping-form');
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', manejarEnvioFormulario);
    }
});
