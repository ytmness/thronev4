...
function generarFiltros(productos) {
    ...
    console.log('Rangos de precio generados:', rangosPrecio);
    ...
    console.log('Tallas disponibles:', tallasOrdenadas);
...
function agregarEventListenersFiltros() {
    document.querySelectorAll('.facet-value').forEach(value => {
        value.addEventListener('click', () => {
            ...
            // Si es el botón "Todas las marcas/precios/tallas"
            if (value.dataset.marca === '' || value.dataset.precio === '' || value.dataset.talla === '') {
                // Si ya está seleccionado, deseleccionar todo
            ...
            // Deseleccionar el botón "Todas" si existe
            const allButton = group.querySelector('.facet-value[data-marca=""], .facet-value[data-precio=""], .facet-value[data-talla=""]');
            if (allButton) {
            ...
            // Si no hay ningún botón seleccionado, seleccionar "Todas"
...
function mostrarPagina(pagina) {
    ...
    productosAMostrar.slice(inicio, fin).forEach(producto => {
...
function aplicarFiltros(filters) {
    ...
    if (!filters.marcas.length && !filters.precios.length && !filters.tallas.length) {
        productosFiltrados = [...productosTenis];
    } else {
        // Filtrar con los criterios seleccionados
        productosFiltrados = productosTenis.filter(producto => {
            let pasaFiltro = true;

            // Verificar marca
            if (filters.marcas && filters.marcas.length > 0) {
                const marcaSeleccionada = filters.marcas.includes(producto.marca);
                console.log(`Producto ${producto.nombre} - Marca ${producto.marca}: ${marcaSeleccionada ? 'Pasa' : 'No pasa'} filtro de marca`);
            ...
            // Verificar precio
            if (filters.precios && filters.precios.length > 0) {
                const precioValido = filters.precios.some(rango => {
                    const [min, max] = rango.split('-').map(Number);
                    let cumplePrecio;
                    if (max) {
                        cumplePrecio = producto.precio >= min && producto.precio < max;
                        console.log(`Producto ${producto.nombre} - Precio ${producto.precio}: Rango ${min}-${max}: ${cumplePrecio ? 'Pasa' : 'No pasa'}`);
                    } else {
                        cumplePrecio = producto.precio >= min;
                        console.log(`Producto ${producto.nombre} - Precio ${producto.precio}: Rango ${min}+: ${cumplePrecio ? 'Pasa' : 'No pasa'}`);
                    }
                    return cumplePrecio;
                });
                if (!precioValido) {
            ...
            if (filters.tallas && filters.tallas.length > 0) {
                const tallaValida = filters.tallas.some(talla => 
                    producto.tallas && producto.tallas[talla] > 0
                );
                console.log(`Producto ${producto.nombre} - Tallas disponibles:`, producto.tallas, `Talla seleccionada: ${filters.tallas.join(', ')}: ${tallaValida ? 'Pasa' : 'No pasa'}`);
                if (!tallaValida) {
                    return false;
                }
            }
...
function actualizarContadorProductos() {
    ...
    document.getElementById('ProductCountMobile').textContent = `${count} producto${count !== 1 ? 's' : ''}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cargarProductos()
        .then(productos => {
            ...
            // Filtrar solo productos de tenis
            productosTenis = productos.filter(p => p.categoria === "Tenis");
            console.log('Productos de tenis encontrados:', productosTenis.length);
            
            if (productosTenis.length === 0) {
                console.error("No se encontraron productos de tenis");
                document.getElementById("productos-grid").innerHTML = 
                    "<p style='grid-column: 1/-1; text-align: center; padding: 20px;'>No hay productos de tenis disponibles en este momento.</p>";
                return;
            }
            ...
            generarFiltros(productosTenis);
            mostrarPagina(1);
            construirPaginacion(productosTenis.length);
            actualizarContadorProductos();
            
            // Mostrar los primeros productos en la consola para verificación
            console.log('Primeros productos de tenis:', productosTenis.slice(0, 3));
        })
        .catch(err => {
