...
function inicializarGruposFiltros() {
    document.querySelectorAll('.facet-group__title').forEach(title => {
        ...
        title.addEventListener('click', (e) => {
            ...
            // Rotar el ícono
            const icon = title.querySelector('.icon-caret');
            if (icon) {
...
function construirPaginacion(totalProductos) {
    ...
    for (let i = 1; i <= totalPaginas; i++) {
        ...
        btn.addEventListener("click", () => {
            ...
            construirPaginacion(productosAccesorios.length);
        });
...
function aplicarFiltros(filters) {
    ...
    if (!filters.marcas.length && !filters.precios.length && !filters.tallas.length) {
        productosFiltrados = [...productosAccesorios];
    } else {
        // Filtrar con los criterios seleccionados
        productosFiltrados = productosAccesorios.filter(producto => {
            let pasaFiltro = true;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cargarProductos()
        .then(productos => {
            ...
            productosAccesorios = productos.filter(p => p.categoria === "Accesorios");
            console.log('Productos de accesorios encontrados:', productosAccesorios.length);
            
            if (productosAccesorios.length === 0) {
                console.error("No se encontraron productos de accesorios");
                document.getElementById("productos-grid").innerHTML = 
                    "<p style='grid-column: 1/-1; text-align: center; padding: 20px;'>No hay productos de accesorios disponibles en este momento.</p>";
                return;
            }
            ...
            generarFiltros(productosAccesorios);
            mostrarPagina(1);
            construirPaginacion(productosAccesorios.length);
            actualizarContadorProductos();
            
            // Mostrar los primeros productos en la consola para verificación
            console.log('Primeros productos de accesorios:', productosAccesorios.slice(0, 3));
        })
        .catch(err => {
